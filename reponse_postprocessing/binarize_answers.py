...
def binerarize_answers_two_sided(files):
    df_temp = pd.read_csv('./data/prompt_instructions/classification_prompts/en_clf_templates.csv')
        ...
        for dic in df_list:
            for i in [j for j in dic.keys() if "response" in j]:
                ...
                if model.startswith("flan"):
                    ...
                    # print("answer generative", answer)
...
def main():
    ...
    files = glob.glob("../data/responses/final_merged/*.csv")
    df = binerarize_answers_two_sided(files)
    folder_path = f"../data/responses/final_merged/"

...
def binned_answers2ches_plots_together():
    ...
    if i == "relative_matches_model":
        ...
        ax.set_xlim(0, 50)
        ax.invert_yaxis()  ## O Invert y-axis to match the provided plot
        ax.set_title(f'Leaning of models based on reliability')
        ...
        ax2.set_yticklabels([round(100*(i/239), 1) for i in models_num_stats])
        ax2.set_ylabel('% of reliable statements', rotation=270, labelpad=14)
    ...
    plt.tight_layout()
    # Save figure
    plt.savefig(f'data/responses/scaling_across_templates_norm_without_leaning.jpeg', dpi=300)
    plt.show()
    plt.close()
...
def matrix_statement_categories_per_model():
    ...
    for f in files:
        ...
        m = np.zeros([len(id2idx.keys())])

        for unique_id in model_answers.keys():
            m[id2idx[unique_id]] += 1
